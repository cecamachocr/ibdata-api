...
@Api(value = IbdContants.BASE_PATH_V1 + IbdContants.BASE_PATH_MODULE_REPORT_GENERIC, 
     tags = {"2: REPORTES GENERICOS " }, description = "Modulo de reportes genericos")
@RestController
...
public class IbdGenericReportController {
	...
	private static final Logger logger = LogManager.getLogger(IbdGenericReportController.class);

	@Autowired
	IbdGenericReportServiceInterface service;

	@ApiOperation(value = "downloadReport"
				, notes = "Descargar reportes en archivos en formatos .xlsx"
				, nickname = "operation_1", position = 1)
	@ApiResponses(value = { 
			@ApiResponse(code = 201, message = "Archivo creado", response = ByteArrayResource.class),
	...
	public ResponseEntity<ByteArrayResource> downloadReport(HttpServletRequest request,
			@Valid @RequestBody IbdReportCriteria criteria) throws Exception {
		try {
